async function updateChartDir() {
    ...
    let response = await fetch('/plotly-2ema?symbol=' + symbol + '&maLengthLF=' + maLengthLF+ '&maLengthHF=' + maLengthHF);
    if (response.status >= 400 && response.status < 600) {
        Plotly.deleteTraces('chartEMA2Chart', [0,1,2,3,4,5]);
        tradeHistory.innerHTML = "Server replied with error; likely an invalid symbol.";
        // throw new Error("Error response from server.");
        return;
    }
    let reply = await response.json();
    Plotly.newPlot('chartEMA2Chart', reply.data, reply.layout);
